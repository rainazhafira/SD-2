...
Node *head = NULL; // karena linked list nya belum ada
Node *tail = NULL;
...
void deleteMiddle(int value) {
    ...
    Node *p = head;
    while(p->next != NULL && p->next->value != value) {// tempatkan si p itu ditempat yg mau dihapus
    p = p -> next;
    }
...
int main() {
    // Contoh list awal
    insertLast(10); // node pertama
    insertLast(20); // node middle
    insertLast(30); // node terakhir
